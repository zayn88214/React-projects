...
export default function CuisinesPage() {
  ...
  const active = params.get('a') || CUISINES[0];

  const { data, isLoading } = useQuery({
    queryKey: ['cuisine', active],
    queryFn: () => recipeApi.listByArea(active),
  });

  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10">
      <h1 className="font-display text-3xl font-semibold mb-2">Cuisines</h1>
      <p className="text-ink-500 dark:text-cream-300 mb-6">Cook your way around the world.</p>

      <div className="flex gap-2 overflow-x-auto pb-2 mb-8">
        {CUISINES.map((c) => (
          <button
            key={c}
            onClick={() => setParams({ a: c })}
            className={cx(
      ...
      {isLoading ? (
        ...
        <EmptyState title="No recipes for this cuisine yet" />
      )}
    </div>
  );
}
